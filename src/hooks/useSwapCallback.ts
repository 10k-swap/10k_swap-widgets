...
import { isAccountInterface } from '../starknet/utils'
...
function useSwapCallArguments(
  ...
  allowedSlippage: Ref<number> | number = INITIAL_SWAP_ALLOWED_SLIPPAGE, // in bips
...
export function useSwapCallback(
  ...
  deadline: number = DEFAULT_DEADLINE_FROM_NOW // in seconds from now
  // recipientAddressOrName: string | null // the ENS name or address of the recipient of the trade, or null if swap should be returned to sender
): Ref<{ state: SwapCallbackState; callback: null | (() => Promise<string>); error: string | null }> {
  const {
    state: { account, chainId, library },
  } = useStarknet()
